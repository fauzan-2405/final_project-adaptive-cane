...
# Konfigurasi jarak
DISTANCE_THRESHOLD = 200  # Batas jarak untuk aktifkan feedback (216 cm)
mode = 'ultrasonic'
...
def set_motor_speed(distance):
    ...
    """
    if distance > 180 or distance <= 0:
        duty_cycle = 0
    else:
        duty_cycle = 100 - (distance / 180) * 95
...
def ultrasonic_mode():
    ...
    while True:
        if not ultrasonic_event.is_set():  # Jika event tidak aktif, thread berhenti
...
def main():
    ...
    # Inisialisasi class
    adc = MCP3008()
    detector = ObjectDetector(model_path="custom_model_lite/detect.tflite", min_conf_threshold=0.7)
